...
@section('content')
                            ...
                            <div class="nk-block-head-content">
                                ...
                                <div class="nk-block-des text-soft">
                                    <ul class="list-inline">
                                        <li>Created At:
                                            <span class="text-base">{{ $sample->created_at->format('d M, Y h:i A') }}</span>
                                        </li>
                                        <li>Status:
                                            <span
                                ...
                                <div class="invoice-bills">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <div class="d-flex gap-3 justify-content-end align-items-center">
                                            ...
                                            @if ($testStatus == 'ALLOTED')
                                                <a href="{{ route('bulk_update_analysis', $sample->tr04_reference_id) }}"
                                                    class="btn btn-success btn-sm bulk-confirm-status" data-action="start"
                                                    data-reference="{{ $sample->tr04_reference_id }}">
                                                    <em class="icon ni ni-play"></em>&nbsp; Start All Tests
                                                </a>
                                            @elseif ($testStatus == 'IN_PROGRESS')
                                                <a href="{{ route('bulk_update_analysis', $sample->tr04_reference_id) }}"
                                                    class="btn btn-warning btn-sm bulk-confirm-status"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @if (($sampleTest->standard->m15_accredited ?? 'NO') === 'YES')
                                                                <br>
                                                                <span class="badge bg-success badge-xxxs">Accredited</span>
                                                            @endif
                                                        </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Confirm before updating single test status
            ...
            // Confirm before bulk updating all tests status
            document.querySelectorAll('.bulk-confirm-status').forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault(); // stop default redirect

                    const url = this.getAttribute('href');
                    const action = this.dataset.action;
                    const referenceId = this.dataset.reference;
                    ...
                    if (action === 'start') {
                        ...
                        text =
                            `Do you want to mark ALL tests for reference #${referenceId} as In Progress? This action will affect all tests in this sample.`;
                        confirmButtonText = 'Yes, Start All Tests';
                    } else {
                        title = 'Complete All Tests?';
                        text =
                            `Do you want to mark ALL tests for reference #${referenceId} as Completed? This action will affect all tests in this sample.`;
                        confirmButtonText = 'Yes, Complete All Tests';
                    }
                    ...
                    Swal.fire({
                        ...
                        cancelButtonText: 'Cancel',
                        confirmButtonColor: action === 'start' ? '#3085d6' : '#28a745',
                        cancelButtonColor: '#d33',
                        reverseButtons: true
                    }).then((result) => {
                        if (result.isConfirmed) {
