...
@section('content')
                                                    ...
                                                    <select name="txt_department_id" class="form-select"
                                                        ...
                                                        @foreach ($departments as $department)
                                                            <option value="{{ $department->m13_department_id }}"
                                                                {{ old('txt_department_id', $test->m13_department_id) == $department->m13_department_id ? 'selected' : '' }}>
                                                                {{ $department->m13_name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <div id="output-matrix-wrapper">
                                                    @php
                                                        // Use m12_output_metrics as per your controller
                                                        $oldOutputs = old(
                                                            'txt_output_matrix',
                                                            json_decode($test->m12_output_metrics, true),
                                                        );
                                                        if (!is_array($oldOutputs)) {
                                                            $oldOutputs = [['name' => '', 'value' => '']];
                                                        }
                                                    @endphp
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-wrap">
                                                    <input type="text" class="form-control" id="txt_charge"
                                                        name="txt_charge"
                                                        value="{{ old('txt_charge', $test->m12_charge) }}" required>
                                                </div>
                                                @error('txt_charge')
                                                    <span class="text-danger">{{ $message }}</span>
                                                @enderror
    ...
    {{-- AJAX and Dynamic Form Script --}}
    <script>
        $(document).ready(function() {
            // Fetch existing stage data from the database
            const existingStages = @json($existingStages);


            // **FIX:** Safely determine which data to use (old input or existing data)
            let oldStagesData = @json(old('stages'));
            if (!oldStagesData || oldStagesData.length === 0) {
                oldStagesData = existingStages;
            }
            ...
            function generateStageConfiguration() {
                ...
                if (stageCount && stageCount > 0 && stageCount <= 50) {
                    ...
                    for (let i = 1; i <= stageCount; i++) {
                        const stageData = oldStagesData[i - 1] || {
                            name: '',
                            inputs: '',
                            outputs: ''
                        };
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="stages[${i-1}][name]" class="form-control"
                                               placeholder="Enter stage ${i} name" value="${stageData.name || ''}" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input name="stages[${i-1}][inputs]" class="form-control"
                                               ...
                                               value="${stageData.inputs || ''}" required>
            ...
            // Event handlers for stages
            $('#txt_input_mode').on('change', toggleStagesField);
            $('#txt_stages').on('input', generateStageConfiguration);

            // For output matrix
            let outputIndex = $('#output-matrix-wrapper .output-row').length;
            ...
            const selectedGroupId = "{{ old('txt_group_id', $test->m11_group_id) }}";

            function fetchGroups(sampleId) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(key, group) {
                                let selected = (group.m11_group_id == selectedGroupId) ?
                                    'selected' : '';
            ...
            // --- Initial Page Load ---
            let initialSampleId = $('#txt_sample_id').val();
            if (initialSampleId) {
                fetchGroups(initialSampleId);
            }
