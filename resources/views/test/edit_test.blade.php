...
@section('content')
                                    ...
                                    <div class="row g-gs">
                                                    ...
                                                    <select name="txt_group_id" class="form-control" id="txt_group_id"
                                                        ...
                                                        @foreach ($groups as $group)
                                                            <option value="{{ $group->m11_group_code }}"
                                                                {{ old('txt_group_id', $test->m11_group_code) == $group->m11_group_code ? 'selected' : '' }}>
                                                                {{ $group->m11_name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <div class="col-md-12">
                                            <div class="preview-block">
                                                ...
                                                <div class="row">
                                                    @php
                                                        $selectedLabSamples = old(
                                                            'lab_sample_ids',
                                                            !empty($test->m14_lab_sample_id)
                                                                ? explode(',', $test->m14_lab_sample_id)
                                                                : [],
                                                        );
                                                    @endphp
                                                        ...
                                                        <div class="col-md-2 mb-2">
                                                            <div class="custom-control custom-checkbox">
                                                                <input type="checkbox" class="custom-control-input"
                                                                    ...
                                                                    {{ in_array($sample->m14_lab_sample_id, $selectedLabSamples) ? 'checked' : '' }}>
                                        ...
                                        {{-- Test Stages --}}
                                        <div class="col-md-4" id="stages-wrapper"
                                            style="{{ old('txt_input_mode', $test->m12_input_mode) == 'MULTI STAGE' ? 'display: block;' : 'display: none;' }}">
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-wrap">
                                                    <input type="number" class="form-control" id="txt_stages"
                                                        name="txt_stages"
                                                        value="{{ old('txt_stages', $test->m12_stages) }}" min="1"
                                                        max="10"
                                                        {{ old('txt_input_mode', $test->m12_input_mode) == 'MULTI STAGE' ? 'required' : '' }}>
                                                </div>
                                                @error('txt_stages')
                                                    <span class="text-danger">{{ $message }}</span>
                                                @enderror
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" name="results" id="results-hidden"
                                                    value="{{ old('results', $test->m12_result) }}">
                                                @error('results')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-wrap">
                                                    <input type="text" class="form-control" id="txt_weight"
                                                        name="txt_weight"
                                                        value="{{ old('txt_weight', $test->m12_weight ?? 0) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            let resultTags = [];
            let isEditing = true;
            let originalTestName = "{{ $test->m12_name }}";
            const searchRoutes = {
            ...
            // Test name existence check (exclude current test)
            $('#txt_name').on('keyup', function() {
                const testName = $(this).val().trim();
                if (testName.length >= 2 && testName !== originalTestName) {
            ...
            function initializeExistingData() {
                ...
                const existingResults = $('#results-hidden').val();
                if (existingResults) {
                    resultTags = existingResults.split(',').filter(Boolean);
                    updateResultsView();
                }

                // Initialize existing standards from comma-separated IDs
                @php
                    $existingStandardIds = !empty($test->m15_standard_id) ? explode(',', $test->m15_standard_id) : [];
                    $existingPrimaryTestIds = !empty($test->m16_primary_test_id) ? explode(',', $test->m16_primary_test_id) : [];
                    $existingSecondaryTestIds = !empty($test->m17_secondary_test_id) ? explode(',', $test->m17_secondary_test_id) : [];
                @endphp

                @if (!empty($existingStandardIds))
                    // Load standards data from server
                    const standardIds = @json($existingStandardIds);
                    if (standardIds.length > 0) {
                        loadExistingStandards(standardIds);
                    }
                @endif

                @if (!empty($existingPrimaryTestIds))
                    // Load primary tests data from server
                    const primaryTestIds = @json($existingPrimaryTestIds);
                    if (primaryTestIds.length > 0) {
                        loadExistingPrimaryTests(primaryTestIds);
                    }
                @endif

                @if (!empty($existingSecondaryTestIds))
                    // Load secondary tests data from server
                    const secondaryTestIds = @json($existingSecondaryTestIds);
                    if (secondaryTestIds.length > 0) {
                        loadExistingSecondaryTests(secondaryTestIds);
                    }
                @endif
            ...
            // New functions to load existing data from server
            function loadExistingStandards(standardIds) {
                $.ajax({
                    url: "{{ route('get_standards_by_ids') }}", // You'll need to create this route
                    type: "GET",
                    data: {
                        ids: standardIds
                    },
                    success: function(standards) {
                        standards.forEach(standard => {
                            selectedStandards.push({
                                id: parseInt(standard.id),
                                name: standard.name,
                                accredited: standard.accredited || 'NO'
                            });
                        });
                        displaySelectedItems('standards');
                    },
                    error: function() {
                        console.error('Error loading existing standards');
                    }
                });
            }

            function loadExistingPrimaryTests(primaryTestIds) {
                $.ajax({
                    url: "{{ route('get_primary_tests_by_ids') }}", // You'll need to create this route
                    type: "GET",
                    data: {
                        ids: primaryTestIds
                    },
                    ...
                    error: function() {
                        console.error('Error loading existing primary tests');
                    }
                });
            }

            function loadExistingSecondaryTests(secondaryTestIds) {
                $.ajax({
                    url: "{{ route('get_secondary_tests_by_ids') }}", // You'll need to create this route
                    type: "GET",
                    data: {
                        ids: secondaryTestIds
                    },
                    success: function(tests) {
                        tests.forEach(test => {
                            selectedSecondaryTests.push({
                                id: parseInt(test.id),
                                name: test.name,
                                primary_test_id: parseInt(test.primary_test_id)
                            });
                        });
                        displaySelectedItems('secondary_tests');
                    },
                    error: function() {
                        console.error('Error loading existing secondary tests');
                    }
                });
            }

            function initializeOldValues() {
                // Initialize results if there are old values (validation errors)
                const oldResults = "{{ old('results') }}";
                if (oldResults && oldResults !== $('#results-hidden').val()) {
                    resultTags = oldResults.split(',').filter(Boolean);
                    updateResultsView();
                }

                // Initialize standards from old input
                @if (old('standard_ids'))
                    selectedStandards = []; // Reset existing data
                    const standardIds = @json(old('standard_ids'));
                    const standardData = @json(old('standard_data', []));
                    standardIds.forEach((id, index) => {
                        const raw = standardData[index] ? JSON.parse(standardData[index]) : {};
                        const data = {
                ...
                // Initialize primary tests from old input
                @if (old('primary_test_ids'))
                    selectedPrimaryTests = []; // Reset existing data
                    ...
                    primaryTestIds.forEach((id, index) => {
                        const raw = primaryTestData[index] ? JSON.parse(primaryTestData[index]) : {};
                        const data = {
                            name: raw.name || `Primary Test ${id}`
                        };
                        selectedPrimaryTests.push({
                            id: parseInt(id),
                            ...data
                ...
                // Initialize secondary tests from old input
                @if (old('secondary_test_ids'))
                    selectedSecondaryTests = []; // Reset existing data
                    ...
                    secondaryTestIds.forEach((id, index) => {
                        ...
                        const raw = secondaryTestData[index] ? JSON.parse(secondaryTestData[index]) : {};
                        const data = {
                            name: raw.name || `Secondary Test ${id}`
                        };
                        selectedSecondaryTests.push({
                            id: parseInt(id),
            ...
            function loadGroups() {
                ...
                const currentGroupId = groupDropdown.val();
                ...
                if (selectedSampleId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            data.forEach(group => {
                                const selected = (group.m11_group_id == currentGroupId || group
                                        .m11_group_id == selectedGroupId) ?
                                    'selected' : '';
                            ...
                            // Update selectedGroupId if value was selected
                            if (groupDropdown.val()) {
                                selectedGroupId = groupDropdown.val();
                            }
                        },
                        error: function() {
                            groupDropdown.empty().append(
            ...
            function checkTestExists(testName) {
                $.ajax({
                    ...
                    data: {
                        name: testName,
                        exclude_id: {{ $test->m12_test_id }} // Exclude current test from check
                    },
