...
@section('content')
                    ...
                    <div class="nk-block-head nk-block-head-sm">
                        <div class="nk-block-head">
                            <div class="nk-block-head-content d-flex justify-content-between align-items-center">
                                <h3 class="nk-block-title page-title">Edit Manuscript (#{{ $manuscript->m22_manuscript_id }})
                                </h3>
                                <div class="text-end">
                    ...
                    <div class="nk-block">
                        <div class="card card-bordered">
                            <div class="card-inner">
                                <form action="{{ route('update_manuscript', $manuscript->m22_manuscript_id) }}"
                                    method="POST" id="editManuscriptForm">
                                    @csrf
                                                ...
                                                <select id="sample_id" name="txt_sample_id" class="form-select" required>
                                                    ...
                                                    @foreach ($samples as $sample)
                                                        <option value="{{ $sample->m10_sample_id }}"
                                                            {{ $manuscript->m10_sample_id == $sample->m10_sample_id ? 'selected' : '' }}>
                                                            {{ $sample->m10_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-control-wrap">
                                                <input type="text" name="txt_name" class="form-control"
                                                    value="{{ $manuscript->m22_name }}" required>
                                                @error('txt_name')
                                                    <span class="text-danger small">{{ $message }}</span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-12 mt-4">
                                            ...
                                            <div class="form-control-wrap">
                                                <textarea class="summernote-basic form-control" name="m22_content" id="m22_content" rows="10"
                                                    placeholder="Write the manuscript report template here...">{{ $manuscript->m22_content }}</textarea>
                                                @error('m22_content')
                                                    <span class="text-danger small">{{ $message }}</span>
                                                @enderror
                                            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            const manuscriptSampleId = "{{ $manuscript->m10_sample_id }}";
            const manuscriptGroupCode = "{{ $manuscript->m11_group_code }}";
            const manuscriptTestNumber = "{{ $manuscript->m12_test_number }}";
            const standardIds = "{{ $manuscript->m15_standard_ids }}".split(',').filter(id => id);

            if (manuscriptSampleId) {
                // Fetch groups
                fetch(`{{ route('get_groups') }}?sample_id=${manuscriptSampleId}`)
                    .then(res => res.json())
                    .then(groups => {
                        groups.forEach(g => {
                            const selected = g.m11_group_code == manuscriptGroupCode ? 'selected' : '';
                            groupSelect.innerHTML +=
                                `<option value="${g.m11_group_code}" ${selected}>${g.m11_name}</option>`;
                        });

                        // Fetch tests
                        if (manuscriptGroupCode) {
                            fetch(`{{ route('get_tests') }}?group_id=${manuscriptGroupCode}`)
                                .then(res => res.json())
                                .then(tests => {
                                    tests.forEach(t => {
                                    ...
                                    if (manuscriptTestNumber) {
                                        ...
                                        fetch(
                                            ...
                                            .then(standards => {
                                                let optionsHTML = '';
                                                standards.forEach(s => {
                                                    const selected = standardIds.includes(s
                                                            .m15_standard_id.toString()) ?
                                                        'selected' : '';
                                                    optionsHTML +=
                                                        `<option value="${s.m15_standard_id}" ${selected}>${s.m15_method}</option>`;
                                                });

                                                if (window.jQuery && $(standardSelect).hasClass(
                                                        'js-select2')) {
                                                    $(standardSelect).html(optionsHTML).trigger(
                                                        'change');
                                                } else {
                                                    standardSelect.innerHTML = optionsHTML;
                                                }
                                        ...
                                        // Pre-load Primary and Secondary Tests from the Test model if they exist
                                        @if (isset($test))
                                            const existingPrimaryIds = "{{ $test->m16_primary_test_id }}"
                                                .split(',').filter(id => id);
                                            const existingSecondaryIds =
                                                "{{ $test->m17_secondary_test_id }}".split(',').filter(
                                                    id => id);
                                            const secondaryAssoc = {!! $test->m17_secondary_test_associations ?: '[]' !!};

                                            // We need to fetch names for these IDs to display them properly
                                            // Since we don't have a direct route to get names by multiple IDs easily here, 
                                            // we will simulate adding them if we had names, or we can fetch them via a custom API if needed.
                                            // For now, we'll just add the IDs. In a real scenario, you'd render the names from the backend directly into a JS array.

                                            // Workaround: Render the names from the backend directly into JS
                                            const primaryTestsData = {!! json_encode(
                                                \App\Models\PrimaryTest::whereIn('m16_primary_test_id', $test ? explode(',', $test->m16_primary_test_id) : [])->get(
                                                    ['m16_primary_test_id as id', 'm16_name as name'],
                                                ),
                                            ) !!};

                                            // Append Primary Tests
                                            primaryTestsData.forEach(pt => {
                                                addSelectedItem('primary_tests', {
                                                    id: pt.id,
                                                    name: pt.name
                                                });
                                            ...
                                            // Append Secondary Tests (matching with associations)
                                            const secondaryTestsData = {!! json_encode(
                                                \App\Models\SecondaryTest::whereIn(
                                                    'm17_secondary_test_id',
                                                    $test ? explode(',', $test->m17_secondary_test_id) : [],
                                                )->get(['m17_secondary_test_id as id', 'm17_name as name']),
                                            ) !!};

                                            secondaryTestsData.forEach(st => {
                                                // Find the primary_test_id for this secondary test from associations
                                                const assoc = secondaryAssoc.find(a => a
                                                    .secondary_test_id == st.id);
                                                if (assoc) {
                                                    addSelectedItem('secondary_tests', {
                                                        id: st.id,
                                                        name: st.name,
                                                        primary_test_id: assoc
                                                            .primary_test_id
                                                    });
                                                }
                                            });
